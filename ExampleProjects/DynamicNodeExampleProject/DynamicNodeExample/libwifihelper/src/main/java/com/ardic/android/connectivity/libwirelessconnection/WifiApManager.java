...
import com.ardic.android.connectivity.libwirelessconnection.listeners.HotspotClientScanFinishListener;
...
public class WifiApManager {
     ...
     * 300
     *
     * @param onlyReachables {@code false} if the list should contain
     *            unreachable (probably disconnected) clients, {@code true}
     *            otherwise
     * @param finishListener, Interface called when the scan method finishes
     */
    public void getClientList(boolean onlyReachables, HotspotClientScanFinishListener finishListener) {
    ...
    public void getClientList(final boolean onlyReachables, final int reachableTimeout,
                              final HotspotClientScanFinishListener finishListener) {

        Runnable runnable = new Runnable() {
            public void run() {


                final ArrayList<HotspotClientScanResult> result = new ArrayList<HotspotClientScanResult>();

                //Java 7 introduced the try-with-resources statement,
                // which implicitly closes Closeables. All resources opened in a try-with-resources statement
                        ...
                        if ((splitted != null) && (splitted.length >= 4)) {
                            ...
                            if (mac.matches("..:..:..:..:..:..")) {
                                ...
                                if (!onlyReachables || isReachable) {
                                    result.add(new HotspotClientScanResult(splitted[0],
                ...
                Runnable myRunnable = new Runnable() {
                    @Override
                    public void run() {
                        finishListener.onClientScanFinish(result);
                    }
                };
